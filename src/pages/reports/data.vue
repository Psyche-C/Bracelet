...
<template>
    ...
    <el-table v-show="flag==1" :data="tableData" :header-row-style="rowClass" border stripe class='table' size='mini' :height='height'>
      ...
      <el-table-column v-for='item in rateReport' :key='item.name' :prop="item.prop" :label="item.name" width='120' align='center'></el-table-column>
    ...
    <div v-show="flag==1" class="block page" :hidden='hideData'>
...
export default {
  ...
  created(){
    ...
    this.findRateNameReport();//获得阶段心率列表名称
    this.findAllData();
  ...
  computed:{
    ...mapGetters(['clazzData','pagesData','gradeData','courseData','tableData','hideData','rateReport']),
  },
  watch:{
    "tableData" : function(now,old){
  ...
  methods:{
    ...
    currentChangeData(val){
  	...
  	gradeChangeData(gid){
      this.form = {grade_id:this.form.grade_id,class_id:'',class_start2:this.form.class_start2,class_end2:this.form.class_end2,course_type_id:this.form.course_type_id}
    ...
    ...mapActions(['findAllData','findRateNameReport','reqClazzData','reqTableData','queryData','downloadClassReport']),
  },
}
</script>
