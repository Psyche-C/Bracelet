  ...
  methods:{
    ...
    //请求班级心率：
      if(this.form.grade_id && this.form.class_id){
        ...
        if(this.form.class_start2){
          var s = Date.parse(this.form.class_start2)/1000;
            this.form.class_end = s + 24*60*60;
        }
        this.reqClazzHeartCurve({class_id:this.form.class_id,class_start:this.form.class_start,class_end:this.form.class_end}).then(()=>{}).catch((error)=>{
          this.$notify({
            type: 'error',
