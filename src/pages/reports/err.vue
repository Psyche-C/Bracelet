...
<template>
  <div class="err">
            ...
            <el-col :span='4'>
              <el-form-item label="" :inline="true">
                <el-select v-model="form.rel_time" placeholder="请选择当前课程">
                  <el-option v-for="item in clazzOnLineErr" :label="item.content" :value="item.rel_time" :key="item.course_name">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-col>
    ...
    <div class="table4">
      <h1 class="notice" v-show="flag==0">请选择查询对象后进行查询···</h1>
      <table v-show="flag==1" class='tableErr' v-loading="loading" element-loading-text="拼命加载中" id='tableErr' cellspacing="0" cellpadding="0" :max-height='height'>
        <thead>
          <tr>
            ...
            <th><div>性别</div></th>
            <th><div>异常状况</div></th>
            <th><div>异常开始时间</div></th>
            <th><div>异常持续时间/秒</div></th>
            <th><div>课程</div></th>
            <th><div>课程内容</div></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for='item in tableErr'>
            ...
            <td><div v-for='j in item.contents'>{{j.content}}</div></td>
            <td><div v-for='j in item.startTimes'>{{j.start_time}}</div></td>
            <td><div v-for='j in item.durations'>{{j.chixu}}</div></td>
            <td>{{item.course_type_name}}</td>
            <td>{{item.course_name}}</td>
          </tr>
        </tbody>
      </table>
      <div v-show="flag==1" class="block page" :hidden='hideErr'>
        <el-pagination @current-change="currentChangeErr" :total="pagesErr" layout="prev,pager,next">
        </el-pagination>
      </div>
    </div>  
  </div>
...
export default {
  ...
  watch:{
    "tableErr" : function(now,old){
      // 如果选中年级，更新对应的班级，否则重置班级
  ...
  methods:{
    ...
    //请求班级：
    gradeChangeErr(gid){
      this.form = {grade_id:this.form.grade_id,class_id:'',class_start2:this.form.class_start2,class_end2:this.form.class_end2,rel_time:''}
    ...
    //请求当前课程：
    clazzChangeErr(cid){
      this.form = {grade_id:this.form.grade_id,class_id:this.form.class_id,class_start2:this.form.class_start2,class_end2:this.form.class_end2,rel_time:''}
      this.reqClazzOnLineErr({class_start2:this.form.class_start2,class_end2:this.form.class_end2,class_id:cid}).then().catch((error)=>{
...
<style scoped>
  ...
  *{
  ...
  #tableErr tr th div,#tableErr tr td div{
    height:35px;
    line-height:35px;
  }
  #tableErr tr th div{
