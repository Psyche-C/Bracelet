<template>
	<div class="list">
		...
		<el-table :data="circle"  style="width:100%" border stripe :header-row-style="rowClass" size='mini' @selection-change="selectChange" align='center' :height='height' ref="table" @row-click='toggleSelection' :default-sort="{prop:'handler_order',order:'ascending'}">
      ...
      <el-table-column prop='tools_name' label="所属工具箱" align='center'></el-table-column>
      <el-table-column prop='handler_order' label="学号" align='center' :sortable='true'></el-table-column>
      ...
      <el-table-column prop="handler_login" label="手环状态" align='center'></el-table-column>
    ...
    <div class="block page" :hidden='hideCircle'>
      ...
      <el-form :model="form2" :rules='rules' class='demo-ruleForm dialog' ref='form2' size='mini'>
        ...
        <el-form-item label="手环编号" :label-width="fw" prop='handler_number'>
          <el-input v-model='form2.handler_number'></el-input>
        </el-form-item>
        <el-form-item label="手环状态" :label-width="fw" prop='handler_login'>
          <el-radio-group v-model="form2.handler_login">
            <el-radio label="良好">良好</el-radio>
            <el-radio label="已坏">已坏</el-radio>
          </el-radio-group>
        </el-form-item>
        <!-- <el-form-item label="启用状态" :label-width="fw" prop='handler_status'>
          <el-radio-group v-model="form2.handler_status">
            <el-radio label="启用">启用</el-radio>
            <el-radio label="已绑定">已绑定</el-radio>
          </el-radio-group>
        </el-form-item> -->
      </el-form>
    ...
    <el-dialog :visible.sync="visible3" center>
      ...
      <el-form :model="form3" :rules='rules3' class='demo-ruleForm dialog' ref='form3' size='mini'>
        <!-- <el-form-item label="序号" :label-width="fw">
          <el-input v-model="form2.num" auto-complete="off"></el-input>
        </el-form-item> -->
        <el-form-item label="手环编号" :label-width="fw" prop='handler_id'>
          <el-select clearable v-model="form3.handler_id">
            <el-option v-for="item in circleTools" :label="item.handler_number" :value="item.handler_id" :key="item.handler_id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="手环状态" :label-width="fw" prop='handler_login'>
          <el-radio-group v-model="form3.handler_login">
            <el-radio label="良好">良好</el-radio>
            <el-radio label="已坏">已坏</el-radio>
          </el-radio-group>
        </el-form-item>
...
export default {
  data() {
    ...
    //  clearShow:true,
      ...
      rules:{
        ...
        handler_login:[
          {required:true,message:'请选择手环状态',trigger: 'change'},
        ],
      },
      rules3:{
        handler_id:[
          {required:true,message:'请选择手环',trigger: 'blur'},
        ],
        handler_login:[
          {required:true,message:'请选择手环状态',trigger: 'change'},
        ],
  ...
  computed:{
    ...mapGetters(['circle','toolsCircle','pagesCircle','hideCircle','circleTools']),
  },
  methods: {
  //分页：
    ...
    batch2(){
      ...
      this.handler_order_id = row.handler_id;
      this.findCircleTools(o);
      this.visible3 = true;
      this.form3 = row;
    },
    ...
    toggleSelection(row,event,column){
      ...
      this.$confirm('此操作将永久删除该条数据，是否继续?', '提示', {
        ...
        this.$notify({title:'成功',type:'info',message:'已取消删除'});          
      });
    },
    batchDelete(){
      ...
      if(ids){
        this.$confirm('此操作将永久删除这些数据，是否继续?','提示',{
          ...
          this.batchDeleteByIdCircle({handlerid_str:ids}).then((result)=>{
            if(result.data.code == 200){
              this.$notify({title:'成功',type:'success',message:'手环信息批量删除成功'});
              this.query();
            }else if(result.data.code == 300){
              this.$notify({title:'删除出错',type:'warning',message:'请先将工具箱和手环解绑后再删除！'});
            }else{
         ...
         this.$notify({type:'info',message:'请先选择手环'});  
      }
    },
  	submit2(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.saveCircle(this.form2).then((result)=>{
            if(result.data.code == 200){
              this.$notify({title:'成功',type:'success',message:'新增成功'});
              this.query();
            }else{
              console.log('手环新增失败，后台返回状态码：'+result.data.code);
            }
          }).catch((error)=>{
            console.log('手环新增失败，后台报错！-->'+error);
          });
          this.visible2=false;
        } else {
    ...
    submit3(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.form3.old_id = this.handler_order_id;
          this.form3.new_id = this.form3.handler_id;
          this.saveCircle(this.form3).then((result)=>{
            if(result.data.code == 200){
              this.$notify({title:'成功',type:'success',message:'修改成功'});
              this.query();
            }else{
              console.log('手环修改失败，后台返回状态码：'+result.data.code);
            }
          }).catch((error)=>{
            console.log('手环修改失败，后台报错！-->'+error);
          });
          this.visible3=false;
        } else {
          console.log('error submit!!');
    ...
    ...mapActions(['findAllCircle','saveCircle','deleteByIdCircle','batchDeleteByIdCircle','findCircle','uploadCircle','findCircleTools']),
  },
}
</script>
