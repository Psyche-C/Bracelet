...
<template>
  <div class="teacher">
      ...
      <el-col :span='14'>
        <el-row type='flex' justify='end'>
          <el-form inline size='mini'>
            ...
            <el-col :span='6'>
              <el-form-item class='but'>
                <el-button type='info' size='small' @click='batchDelete2' class="delete"><!-- <i class="fa fa-plus-circle"></i> -->批量删除</el-button>
              </el-form-item>
            </el-col>
          </el-form>
        </el-row>
    ...
    <el-table :data="teachers" style="width: 100%" class='table' border stripe :header-row-style="rowClass" size='mini' @selection-change="selectArr" align='center' :height='height' ref="table" @row-click='toggleSelection'>
      ...
      <el-table-column prop="teach_status" label="启用状态" align='center' width='80'></el-table-column>
    ...
    <el-dialog :visible.sync="visibleAdmin" center>
      ...
      <el-tree ref="auth" :data="teacherAdmin" node-key="module_id" :default-checked-keys='teaadminOne' :props="props" show-checkbox></el-tree>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  created(){
    ...
    this.teachUpUrl = bp+'/manager/teacher_up?token='+this.token;
  ...
  computed:{
    ...mapGetters(['teachers','pageTeach','teacherAdmin','hideTeach','teaadminOne']),
  },
  methods:{
    selectArr(val){
    ...
    downloadTeach(){//下载模版
    //跳转到下载地址：
      window.location.href = bp+'/manager/teacher_download?token='+this.token;
    },
    ...
    batchDelete2(){
      ...
      for(var i=0;i<this.selectTeacher.length;i++){
        i == this.selectTeacher.length-1 ? str += this.selectTeacher[i] : str += (this.selectTeacher[i]+'@');
      }
      this.$confirm('此操作将永久删除选中数据，是否继续?','提示', {
        ...
        this.batchDeleteByIdTch({teachid_str:str}).then((result)=>{
          if(result.data.code == 200){
            this.$notify({
              title:'成功',
              type: 'success',
            ...
            console.log('教师信息批量删除失败，后台返回状态码：'+result.data.code);
          }
        }).catch((error)=>{
          console.log('教师信息批量删除失败，后台报错！-->'+error);
        });
      }).catch(() => {
        this.$notify({
          type: 'info',
    ...
    single2(){
      this.visible2=true;
      this.title2='单个新增教师';
      this.form2={teach_gender:'女',teach_status:'启用',teach_tel:'',};
    },
    batch2(){
      this.visible2s=true;
    ...
    admin2(){
      if(this.selectTeacher.length == 0){
        ...
        // 获取当前选中用户权限
        this.teaadminOneR({user_id:this.selectTeacher[0]}); 
      }else if(this.selectTeacher.length > 1){//不选老师不弹。
        this.visibleAdmin=true;
        this.titleAdmin='用户授权';
        this.teaadminOneRAll();
      }
    },
    submit2(formName){
      var flag = 0;
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.form2.teach_passwd = Base64.encode(this.form2.teach_passwd);//加密。
          this.saveTch(this.form2).then((result)=>{
            if(result.data.code == 200){
              ...
              // 如果是修改执行局部更新
              if(this.form2.teach_id==undefined){
                this.query();
              }
              flag = 0;
   ...
   /* handleRemove(file, fileList) {
    ...
    ...mapActions(['findAllTch','saveTch','deleteByIdTch','findTch','queryTeach','grantTch','teaadminOneR','teaadminOneRAll','batchDeleteByIdTch']),
  }
}
</script>
